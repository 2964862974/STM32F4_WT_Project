...
void USART2_IRQHandler(void)
{
    ...
    {
        if (rxlen < sizeof(rxbuf) - 1)//预防缓冲区溢出
        {
            rxbuf[rxlen++] = USART_ReceiveData(USART2);//每接收一个字符+1

            if (rxbuf[rxlen - 1] == '\n')//当接收到换行符时，认为一行数据接收完成
            {
                rxbuf[rxlen] = '\0';//添加字符串结束符
                at_ack_t ack = match_internal_ack(rxline);//尝试匹配当前数据是否为AT命令的响应
                rxline = rxbuf + rxlen;//将line指向下一个数据的开始位置
                if (ack != AT_ACK_NONE)//如果匹配成功，说明接收到了AT命令的响应
                {
                    at_ack_received = ack;
